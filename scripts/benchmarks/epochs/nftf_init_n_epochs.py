...
def build_iteration_contexts() -> list[dict]:
    base_contexts = [
        ("DTF w/ Init", CONTEXT_WITH_NFTF),
        ("DTF w/o Init", CONTEXT_WITH_NFTF_NO_PREFIT),
        ("No DTF", CONTEXT_WITHOUT_NFTF),
    ]
    contexts = []
    for iterations in TRAN_ITERATION_VALUES:
        for base_name, base_params in base_contexts:
            params = deepcopy(base_params)
            params["tran_params"]["iterations"] = iterations
            contexts.append(
                {
                    "name": f"{base_name} (tran iterations={iterations})",
                    "params": params,
                }
            )
    return contexts
...
def main() -> None:
    ...
    contexts = build_iteration_contexts()
