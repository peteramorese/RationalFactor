...
if __name__ == "__main__":
    ...
    phi_basis =  BetaBasis.set_init(system.dim(), n_basis=n_basis, offsets=torch.tensor([0.0, 0.5]))
    psi_basis =  BetaBasis.set_init(system.dim(), n_basis=n_basis, offsets=torch.tensor([0.0, 0.5]))
    psi0_basis = BetaBasis.set_init(system.dim(), n_basis=n_basis, offsets=torch.tensor([0.0, 0.5]))

    # Create separable domain transformation
    domain_tf = ErfSeparableTF(system.dim())

    # Create and train the transition model
    tran_model = CompositeDensityModel(domain_tf, QuadraticRFF(phi_basis, psi_basis))
    ...
    init_model = CompositeDensityModel(domain_tf, QuadraticFF.from_rff(tran_model, psi0_basis))
