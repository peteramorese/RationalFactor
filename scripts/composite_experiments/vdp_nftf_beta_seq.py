...
from rational_factor.models.domain_transformation import MaskedRQSNFTF, ErfSeparableTF
...
if __name__ == "__main__":
    ...
    if use_dtf:
        tran_params = {
            "n_epochs_per_group": [15, 5, 5], # dtf_params, basis, weights
            "iterations": 10,
            "lr_basis": 5e-2,
            "lr_weights": 1e-2,
            "lr_dtf": 1e-3,
    ...
    pre_tran_model = CompositeConditionalModel([wrap_tf], LinearRFF(phi_basis, psi_basis)).to(device)
    ...
    optimizers ={"basis": torch.optim.Adam([
            {'params': pre_tran_model.conditional_density_model.basis_params(), 'lr': pre_train_tran_params["lr_basis"]}, 
            #{'params': pre_tran_model.domain_tfs.parameters(), 'lr': pre_train_tran_params["lr_wrap"]}
        ]),
        "weights": torch.optim.Adam(pre_tran_model.conditional_density_model.weight_params(), lr=pre_train_tran_params["lr_weights"])}

    tran_model, best_loss_tran, training_time_tran = train.train_iterate(pre_tran_model,
        xp_dataloader,
        {"mle": mle_loss_fn, "var_reg": var_reg_loss_fn}, 
        optimizers,
        epochs_per_group=pre_train_tran_params["n_epochs_per_group"],
        iterations=pre_train_tran_params["iterations"],
        verbose=True,
        use_best="mle")
    print("Done! \n")
