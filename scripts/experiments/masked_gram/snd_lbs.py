...
def _make_free_basis(
    *,
    ...
    flow_hidden: int,
    device: torch.device,
) -> NormalizedProductPairBasis | None:
    """Free pair on ``R^{rest_dim}``: NF product + additive VP splitter.

    Returns ``None`` when ``rest_dim == 0`` (identity free pair in the mask).
    """
    if rest_dim == 0:
        return None
    ...
    if rest_dim < 2:
        raise ValueError(
            "ConditionalVolumePreservingFlow (additive coupling) requires rest_dim >= 2; "
            f"got rest_dim={rest_dim}"
        )
    ...
    return NormalizedProductPairBasis(product, splitter, embedding).to(device)
...
if __name__ == "__main__":
    ...
    # Free pair on R^{d-1}: NF product + additive VP splitter (None when rest_dim=0).
    ...
    output_dir = Path("figures/masked_gram/snd_lbs")
    ...
    _plot_snd_1d_belief_trajectory(
        ...
        beliefs_out_path,
        n_grid=400,
        title="SND LBS: propagated beliefs vs test trajectories",
    )
    ...
    _plot_snd_conditional_true_vs_learned(
        ...
        problem=problem,
        n_grid=200,
        title="SND LBS: true vs learned p(x'|x)",
    )
