...
if __name__ == "__main__":
    ...
    optimizers = {
        "basis": torch.optim.Adam(
            [
                #{"params": param_group_iter((cell_widths, alpha_params)), "lr": tran_params["lr_basis"]},
                {"params": param_group_iter((alpha_params,)), "lr": tran_params["lr_basis"]},
    ...
    _plot_snd_1d_belief_trajectory(
        ...
        beliefs_out_path,
        n_grid=400,
        title="SND DJS: propagated beliefs vs test trajectories",
    )
    ...
    _plot_snd_conditional_true_vs_learned(
        ...
        problem=problem,
        n_grid=200,
        title="SND DJS: true vs learned p(x'|x)",
    )
