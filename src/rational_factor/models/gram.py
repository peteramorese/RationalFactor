...
class Omega2Gram:
    """Uniform Ω₂ interface over a dense tensor or a structured factor.

    ``Omega2`` may be a dense ``(..., n, m)`` tensor, a ``Rank1PlusDiagonal``,
    or another ``Omega2Gram``. Callers use ``matvec`` / ``rev_matvec`` and do
    not branch on the representation: dense grams use einsum, structured grams
    use their linear-time product.
    """

    def __init__(self, Omega2: torch.Tensor | Rank1PlusDiagonal | Omega2Gram):
        if isinstance(Omega2, Omega2Gram):
            self._dense = Omega2._dense
            self._structured = Omega2._structured
            return
        if isinstance(Omega2, Rank1PlusDiagonal):
            self._dense = None
            self._structured = Omega2
            return
        dense = torch.as_tensor(Omega2)
        if dense.dim() < 2:
            raise ValueError(f"dense Omega2 must have shape (..., n, m), got {tuple(dense.shape)}")
        self._dense = dense
    ...
    def dtype(self) -> torch.dtype:
        return self._structured.dtype if self.is_structured else self._dense.dtype

    @property
    def device(self) -> torch.device:
        return self._structured.device if self.is_structured else self._dense.device

    @property
    def shape(self) -> torch.Size:
        return self._structured.shape if self.is_structured else self._dense.shape

    @property
    def T(self) -> Omega2Gram:
        if self.is_structured:
            return Omega2Gram(self._structured.T)
        return Omega2Gram(self._dense.transpose(-2, -1))

    def to_dense(self) -> torch.Tensor:
        if self.is_structured:
            return self._structured.to_dense()
        return self._dense

    def diag(self) -> torch.Tensor:
        """Main-diagonal vector ``(..., min(n, m))``."""
        if self.is_structured:
            return self._structured.d + self._structured.u * self._structured.v
        return self._dense.diagonal(dim1=-2, dim2=-1)

    def sum(self) -> torch.Tensor:
        """Sum of all matrix entries (matches ``torch.sum`` on the dense gram)."""
        if not self.is_structured:
            return self._dense.sum()
        s = self._structured
        return s.d.sum() + (s.u.sum(dim=-1) * s.v.sum(dim=-1)).sum()

    def matvec(self, x: torch.Tensor) -> torch.Tensor:
        """``Ω @ x``. ``x`` is ``(..., m)`` or ``(..., m, k)``."""
        if self.is_structured:
            return self._structured.matvec(x)
        x = torch.as_tensor(x, dtype=self.dtype, device=self.device)
        n_in = self._dense.shape[-1]
        batch_ndim = self._dense.dim() - 2
        if x.dim() >= batch_ndim + 2 and x.shape[-2] == n_in:
            return torch.einsum("...ij,...jk->...ik", self._dense, x)
        if x.shape[-1] == n_in:
            return torch.einsum("...ij,...j->...i", self._dense, x)
        raise ValueError(
    ...
    def rev_matvec(self, x: torch.Tensor) -> torch.Tensor:
        """``Ωᵀ @ x``. ``x`` is ``(..., n)`` or ``(..., n, k)``."""
        if self.is_structured:
            return self._structured.T.matvec(x)
        x = torch.as_tensor(x, dtype=self.dtype, device=self.device)
        n_out = self._dense.shape[-2]
        batch_ndim = self._dense.dim() - 2
        if x.dim() >= batch_ndim + 2 and x.shape[-2] == n_out:
            return torch.einsum("...ji,...jk->...ik", self._dense, x)
        if x.shape[-1] == n_out:
            return torch.einsum("...ij,...i->...j", self._dense, x)
        raise ValueError(
            f"x must have shape (..., {n_out}) or (..., {n_out}, k), got {tuple(x.shape)}"
        )

    def mul_diag_left(self, a: torch.Tensor) -> Omega2Gram:
        """Left-multiply by ``diag(a)``."""
        if self.is_structured:
            return Omega2Gram(self._structured.mul_diag_left(a))
        a = torch.as_tensor(a, dtype=self.dtype, device=self.device)
        return Omega2Gram(a.unsqueeze(-1) * self._dense)

    def mul_diag_right(self, a: torch.Tensor) -> Omega2Gram:
        """Right-multiply by ``diag(a)``."""
        if self.is_structured:
            return Omega2Gram(self._structured.mul_diag_right(a))
        a = torch.as_tensor(a, dtype=self.dtype, device=self.device)
    ...
    def __repr__(self) -> str:
        kind = "structured" if self.is_structured else "dense"
        return f"Omega2Gram({kind}, shape={tuple(self.shape)}, dtype={self.dtype}, device={self.device})"
